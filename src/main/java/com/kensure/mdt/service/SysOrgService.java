...
public class SysOrgService extends JSBaseService {
	...
	public List<SysTree> selectTree(AuthUser user) {
		Map<String, Object> parameters = MapUtils.genMap();
		setOrgLevel(parameters, user);
		List<SysOrg> orgList = selectByWhere(parameters);

		Map<String, List<SysTree>> treeMap = new HashMap<>();
		for (SysOrg org : orgList) {
			List<SysTree> lst = treeMap.get(org.getPid());
			if (lst == null) {
				lst = new ArrayList<>();
				treeMap.put(org.getPid(), lst);
			}
			lst.add(org.toTree());
		}
		List<SysTree> treelist = treeMap.get("1");
		for (SysTree tree : treelist) {
			deepTree(tree, treeMap);
		}
		return treelist;
	}

	private void deepTree(SysTree tree, Map<String, List<SysTree>> treeMap) {
		List<SysTree> list = treeMap.get(tree.getId());
		if (CollectionUtils.isNotEmpty(list)) {
			tree.getChildren().addAll(list);
			for (SysTree cd : list) {
				deepTree(cd, treeMap);
			}
		}
